...
#define OSTREE_SIGN_ED25519_NAME crypto_sign_PRIMITIVE
...
static void
ostree_sign_ed25519_finalize (GObject *gobject)
{
  g_debug ("%s enter", __FUNCTION__);

  OstreeSignEd25519 *self = ostree_sign_ed25519_get_instance_private(OSTREE_SIGN_ED25519(gobject));

  g_free (self->secret_key);
  g_free (self->public_key);

  G_OBJECT_CLASS (ostree_sign_ed25519_parent_class)->finalize(gobject);
}
  ...
  for (gsize i = 0; i < g_variant_n_children(signatures); i++)
    {
      ...
      g_debug("Read signature %ld: %s", i, g_variant_print(child, TRUE));
                                     ...
                                     GError **error)
{
  ...
  sign->public_key = (guchar *) g_strdup (g_variant_get_fixed_array (public_key, &n_elements, sizeof(guchar)));
